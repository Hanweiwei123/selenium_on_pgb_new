...
describe "TC_005: Register an free plan account with Github ID" do 
	...
	@driver.manage.timeouts.implicit_wait = 30
        ...
        register_locator(:github_btn).click; sleep 5
    end
    ...
    it "IT_001: direct to '/apps' page when sign in successfully with github id(which is connected to pgb), " do 
        fill_in sign_in_github_locator(:username_or_email), :with => $data[:user][$lang][:github_id_only][:id]
        fill_in sign_in_github_locator(:password), :with => $data[:user][$lang][:github_id_only][:password]
        sign_in_github_locator(:sign_in_btn).click;  sleep 10
    ...
    it "IT_002: Got an error message('Existing Registration Found') when use account that can log in both. " do  
        fill_in sign_in_github_locator(:username_or_email), :with => $data[:user][$lang][:email_that_login_both][:id]
        fill_in sign_in_github_locator(:password), :with => $data[:user][$lang][:email_that_login_both][:password]
    ...
    it "IT_003: Got an error message when proceed without selecting a country" do 
        fill_in sign_in_github_locator(:username_or_email), :with => $data[:user][$lang][:adobe_id_free_final_step][:id]
        fill_in sign_in_github_locator(:password), :with => $data[:user][$lang][:adobe_id_free_final_step][:password]
        sign_in_github_locator(:sign_in_btn).click; sleep 5
