...
describe "TC_001: New app(s) with free account" do
  ...
  context "--- with Adobe ID - free account" do
    before(:all) do
      ...
      puts "+ <TC_001> context 1 before all inside --- end"
    end
    ...
    it "IT_003: got an warning message when submit an invalid .git address" do
      puts "IT_" + @order_of_it.to_s
      warning = @new_app_page.paste_an_invaild_git_repo("abcd")
    ...
    it "IT_007: the number of apps was not the same as before, after delete one app" do
      puts "IT_" + @order_of_it.to_s
  ...
  context "--- with Adobe ID - free account - connected github" do
    before(:all) do
      ...
      @new_app_page = NewAppPage.new(
          :driver => @driver,
          :base_url => @base_url,
          :user => {:id => $data[:user][$lang][:adobe_id_free_connected_github][:id], :password => $data[:user][$lang][:adobe_id_free_connected_github][:password]})
      sleep 10
      puts "+ <TC_001> context 2 before all inside --- end"
    end
    ...
    it "IT_008: the number of items of the dropdown list did not equal 0" do
      puts "IT_" + @order_of_it.to_s
      if @new_app_page.new_app_btn_display?
        new_app_locator(:new_app_btn).click
        puts "new_app_btn.click"
      end
      new_app_locator(:paste_git_repo_dropdown).click
      sleep 3
      li_count = @driver.find_elements(:xpath => "//*[@id='new-app']/form/div[2]/div[1]/div/ul/li").count
      puts "+li count: #{li_count}"
      li_count.should_not eql 0
    end

    it "IT_009: check if the placeholder of 'find existing repo / paste .git repo' exists" do
      puts "IT_" + @order_of_it.to_s
      if @new_app_page.new_app_btn_display?
        new_app_locator(:new_app_btn).click
        puts "new_app_btn.click"
      end
      new_app_locator(:paste_git_repo_input).attribute('placeholder').to_s.should eql $data[:str][$lang][:PGB_find_existing_repo_or_paste_git_repo]
    end

    it "IT_010: got an errors when pasting a invalid .git address" do
      puts "IT_" + @order_of_it.to_s
      if @new_app_page.new_app_btn_display?
        new_app_locator(:new_app_btn).click
        puts "new_app_btn.click"
      end
      warning = @new_app_page.paste_an_invaild_git_repo("亜印gっをげwにwpw声wんw儀栄wペイ儀絵印rgる")
      warning.should eql $data[:str][$lang][:PGB_not_a_valid_github_url]
    end

    it "IT_011: there be an matched item from the dropdown list when entering some letters" do
      puts "IT_" + @order_of_it.to_s
      if @new_app_page.new_app_btn_display?
        new_app_locator(:new_app_btn).click
        puts "new_app_btn.click"
      ...
      new_app_locator(:paste_git_repo_input).send_keys("sa")
      the_first_item = new_app_locator(:paste_git_repo_dropdown_list)
      puts "The first matched item: #{the_first_item.text}"
      the_first_item.text.should eql $data[:str][$lang][:PGB_the_first_matched_item]
    end

    it "IT_012: there be not any matched apps in the list when enter some specific letters in the placeholder" do
      puts "IT_" + @order_of_it.to_s
      if @new_app_page.new_app_btn_display?
        new_app_locator(:new_app_btn).click
        puts "new_app_btn.click"
      ...
      new_app_locator(:paste_git_repo_input).clear
      new_app_locator(:paste_git_repo_input).send_keys("sss")
      the_first_item = new_app_locator(:paste_git_repo_dropdown_list)
      puts "The first item: #{the_first_item.text}"
      the_first_item.text.should eql $data[:str][$lang][:PGB_no_match_item]
    end

    it "IT_013: the number of apps did not equal to 0 after creating app by selecting and clicking one from the matched items" do
      puts "IT_" + @order_of_it.to_s
      if @new_app_page.new_app_btn_display?
        new_app_locator(:new_app_btn).click
        puts "new_app_btn.click"
      ...
      new_app_locator(:paste_git_repo_input).clear
      new_app_locator(:paste_git_repo_input).send_keys("start")
      the_first_item = new_app_locator(:paste_git_repo_dropdown_list)
    ...
    it "IT_014: the number of apps was not the same as before, after delete one app" do
      puts "IT_" + @order_of_it.to_s
